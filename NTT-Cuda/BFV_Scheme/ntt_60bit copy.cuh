...
{  // performs single NTT operation
    if (n == 32768)
    {
        ...
        /*test Single*/
        CTBasedNTTInnerSingle<1, 32768> << <1, 1024, 4 * 4096 * sizeof(unsigned long long), stream1 >> > (device_a, q, mu, bit_length, psi_powers);  // rest of the iterations, single-kernel
    }
    else if (n == 16384)
    {
        ...
        /*test Single*/
        CTBasedNTTInnerSingle<1, 16384> << <1, 1024, 4 * 4096 * sizeof(unsigned long long), stream1 >> > (device_a, q, mu, bit_length, psi_powers);  // rest of the iterations, single-kernel
    }
    else if (n == 8192)
    {
